...
async function setup() {
  ...
  for (const table of tables) {
    try {
      ...
      console.log(`✓ ${table.name}: ${res.status === 200 || res.status === 201 ? 'OK' : 'JA EXISTE'}`);
    } catch (e) {
      console.log(`✓ ${table.name}: ERRO (provavelmente ja existe)`);
    }
  }

  console.log('\nCriando bucket de storage...');
  try {
    const bucketRes = await postRequest('/storage/v1/bucket', { id: 'local-market', name: 'local-market', public: true });
    console.log(`✓ Bucket: ${bucketRes.status === 200 || bucketRes.status === 201 ? 'OK' : 'JA EXISTE'}`);
  } catch (e) {
    console.log(`✓ Bucket: ERRO (provavelmente ja existe)`);
  }
